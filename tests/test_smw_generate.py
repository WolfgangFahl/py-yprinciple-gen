...
class TestSMWGenerate(BaseSemanticMediawikiTest):
    ...
    def getMarkup(
        ...
        debug: bool = False,
        wikiId="cr",
        context_name="CrSchema",
        topicNames=["Event"],
        target_keys=["category", "concept", "form", "help", "listOf", "template"],
    ):
        """
    ...
    def test_Issue13_ExternalIdentifer_Link_handling(self):
        ...
        debug = self.debug
        # debug=True
        for _topicname, _target_key, _smwTarget, markup in self.getMarkup(
            debug, target_keys=["template"]
        ):
            if debug:
                print(markup)
    ...
    def test_Issue28_viewmode_masterdetail(self):
        ...
        debug = self.debug
        # debug=True
        for _topicname, target_key, _smwTarget, markup in self.getMarkup(
            wikiId="wiki",
            context_name="MetaModel",
            topicNames=["Context"],
            target_keys=["template"],
            debug=debug,
        ):
    ...
    def test_Issue29_TopicLink_separator(self):
        ...
        # debug=True
        for _topicname, target_key, _smwTarget, markup in self.getMarkup(
            topicNames=["Paper"], target_keys=["template"], debug=debug
        ):
            if target_key == "template":
                if debug:
                    print(markup)
